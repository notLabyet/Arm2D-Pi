...
#define QMI8658_FIRE_GRAVITY_FILTER_DIV               8
#define QMI8658_FIRE_ACCEL_FILTER_DIV                 4
#define QMI8658_FIRE_ACCEL_DEADZONE_G10000            250
#define QMI8658_FIRE_ACCEL_LIMIT_G10000               10000
#define QMI8658_FIRE_GRAVITY_MIN_G10000               1000
#define QMI8658_Q15_MAX                               32767
...
typedef struct qmi8658_motion_state_t {
    ...
    int32_t fire_gravity[3];
    int32_t fire_acceleration[2];
    qmi8658_fire_motion_t fire_motion;
    uint32_t last_sample_ms;
    int16_t max_x;
    int16_t max_y;
    uint8_t calibration_count;
    uint8_t fire_gravity_valid;
    uint8_t tap_event_latched;
} qmi8658_motion_state_t;

static qmi8658_motion_state_t s_tMotion;

/* Compatibility snapshot consumed by the existing 3D loader. */
int16_t DATA_GY_ACC_RAW[6];
volatile uint32_t DATA_GY_ACC_SAMPLE_SEQ;
...
static uint32_t qmi8658_motion_abs_u32(int32_t value)
{
    if (value >= 0) {
        return (uint32_t)value;
    }

    return 0u - (uint32_t)value;
}

static int16_t qmi8658_motion_ratio_to_q15(int32_t value, uint32_t divisor)
{
    int64_t scaled;
    ...
    scaled = ((int64_t)value * QMI8658_Q15_MAX) / divisor;
    if (scaled > QMI8658_Q15_MAX) {
        scaled = QMI8658_Q15_MAX;
    } else if (scaled < -QMI8658_Q15_MAX) {
        scaled = -QMI8658_Q15_MAX;
    }
...
static int16_t qmi8658_motion_accel_to_q15(int32_t value)
{
    value = qmi8658_motion_limit_i32(value,
                                     QMI8658_FIRE_ACCEL_LIMIT_G10000);
    return qmi8658_motion_ratio_to_q15(
        value,
        QMI8658_FIRE_ACCEL_LIMIT_G10000);
}

static void qmi8658_motion_update_fire_input(int32_t ax,
                                              int32_t ay,
                                              int32_t az)
{
    int32_t acceleration_x;
    int32_t acceleration_y;
    int32_t acceleration_depth;
    int32_t screen_gravity_x;
    int32_t screen_gravity_y;
    uint32_t abs_gravity_x;
    uint32_t abs_gravity_y;
    uint32_t gravity_norm;
    uint32_t disturbance;

    if (!s_tMotion.fire_gravity_valid) {
        s_tMotion.fire_gravity[0] = ax;
        s_tMotion.fire_gravity[1] = ay;
        s_tMotion.fire_gravity[2] = az;
        s_tMotion.fire_gravity_valid = 1u;
    } else {
        s_tMotion.fire_gravity[0] +=
            (ax - s_tMotion.fire_gravity[0]) /
            QMI8658_FIRE_GRAVITY_FILTER_DIV;
        s_tMotion.fire_gravity[1] +=
            (ay - s_tMotion.fire_gravity[1]) /
            QMI8658_FIRE_GRAVITY_FILTER_DIV;
        s_tMotion.fire_gravity[2] +=
            (az - s_tMotion.fire_gravity[2]) /
            QMI8658_FIRE_GRAVITY_FILTER_DIV;
    }

#if QMI8658_FIRE_USE_PRISM
    screen_gravity_x = s_tMotion.fire_gravity[1];
    screen_gravity_y = s_tMotion.fire_gravity[2];
    acceleration_x = ay - s_tMotion.fire_gravity[1];
    acceleration_y = az - s_tMotion.fire_gravity[2];
    acceleration_depth = ax - s_tMotion.fire_gravity[0];
#else
    screen_gravity_x = s_tMotion.fire_gravity[0];
    screen_gravity_y = s_tMotion.fire_gravity[1];
    acceleration_x = ax - s_tMotion.fire_gravity[0];
    acceleration_y = ay - s_tMotion.fire_gravity[1];
    acceleration_depth = az - s_tMotion.fire_gravity[2];
#endif

    acceleration_x = qmi8658_motion_apply_deadzone_i32(
        acceleration_x,
        QMI8658_FIRE_ACCEL_DEADZONE_G10000);
    acceleration_y = qmi8658_motion_apply_deadzone_i32(
        acceleration_y,
        QMI8658_FIRE_ACCEL_DEADZONE_G10000);
    acceleration_depth = qmi8658_motion_apply_deadzone_i32(
        acceleration_depth,
        QMI8658_FIRE_ACCEL_DEADZONE_G10000);

    s_tMotion.fire_acceleration[0] +=
        (acceleration_x - s_tMotion.fire_acceleration[0]) /
        QMI8658_FIRE_ACCEL_FILTER_DIV;
    s_tMotion.fire_acceleration[1] +=
        (acceleration_y - s_tMotion.fire_acceleration[1]) /
        QMI8658_FIRE_ACCEL_FILTER_DIV;

    abs_gravity_x = qmi8658_motion_abs_u32(screen_gravity_x);
    abs_gravity_y = qmi8658_motion_abs_u32(screen_gravity_y);
    gravity_norm = (abs_gravity_x > abs_gravity_y)
                 ? abs_gravity_x + (abs_gravity_y / 2u)
                 : abs_gravity_y + (abs_gravity_x / 2u);

    if (gravity_norm >= QMI8658_FIRE_GRAVITY_MIN_G10000) {
        s_tMotion.fire_motion.gravity_x_q15 =
            qmi8658_motion_ratio_to_q15(screen_gravity_x,
                                        gravity_norm);
        s_tMotion.fire_motion.gravity_y_q15 =
            qmi8658_motion_ratio_to_q15(screen_gravity_y,
                                        gravity_norm);
    } else {
        s_tMotion.fire_motion.gravity_x_q15 = 0;
        s_tMotion.fire_motion.gravity_y_q15 = QMI8658_Q15_MAX;
    }

    s_tMotion.fire_motion.acceleration_x_q15 =
        qmi8658_motion_accel_to_q15(s_tMotion.fire_acceleration[0]);
    s_tMotion.fire_motion.acceleration_y_q15 =
        qmi8658_motion_accel_to_q15(s_tMotion.fire_acceleration[1]);

    disturbance = qmi8658_motion_abs_u32(s_tMotion.fire_acceleration[0]);
    if (qmi8658_motion_abs_u32(s_tMotion.fire_acceleration[1]) > disturbance) {
        disturbance = qmi8658_motion_abs_u32(s_tMotion.fire_acceleration[1]);
    }
    if (qmi8658_motion_abs_u32(acceleration_depth) > disturbance) {
        disturbance = qmi8658_motion_abs_u32(acceleration_depth);
    }
    if (disturbance > QMI8658_FIRE_ACCEL_LIMIT_G10000) {
        disturbance = QMI8658_FIRE_ACCEL_LIMIT_G10000;
    }
    s_tMotion.fire_motion.disturbance_q15 =
        (uint16_t)qmi8658_motion_ratio_to_q15(
            (int32_t)disturbance,
            QMI8658_FIRE_ACCEL_LIMIT_G10000);
}
...
bool qmi8658_motion_poll(uint32_t current_ms)
{
    ...
    if ((uint32_t)(current_ms - s_tMotion.last_sample_ms) <
        QMI8658_MOTION_SAMPLE_PERIOD_MS) {
        return false;
    }

    s_tMotion.last_sample_ms = current_ms;
    if (!QMI8658A_ReadData(raw_data)) {
        return false;
    }

    memcpy(DATA_GY_ACC_RAW, raw_data, sizeof(DATA_GY_ACC_RAW));
    qmi8658_motion_update_raw(raw_data);
    DATA_GY_ACC_SAMPLE_SEQ++;
    return true;
}
...
{
    if ((NULL == motion) || !s_tMotion.fire_gravity_valid) {
        return false;
    }

    *motion = s_tMotion.fire_motion;
    return true;
}
...
void qmi8658_motion_update_raw(int16_t const raw_data[6])
{
    ...
    qmi8658_motion_update_fire_input(ax, ay, az);
