...
#if !PICO_RISCV
static inline bool rp2040_sdio_in_exception(void)
{
    uint32_t ipsr;
...
static sdio_status_t check_sdio_write_response(uint32_t card_response)
{
    ...
    {
        EMSG_PRINTF("SDIO card reports write CRC error, status %x\n", card_response);
        return SDIO_ERR_WRITE_CRC;    
    }
    else if (wr_status == 6)
    {
        EMSG_PRINTF("SDIO card reports write failure, status %x\n", card_response);
        return SDIO_ERR_WRITE_FAIL;    
    }
    else
    {
        EMSG_PRINTF("SDIO card reports unknown write status %x\n", card_response);
        return SDIO_ERR_WRITE_FAIL;    
    }
}
