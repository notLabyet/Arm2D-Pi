...
uint8_t sd_sdio_type(sd_card_t *sd_card_p);

static uint32_t sd_sdio_card_address(sd_card_t *sd_card_p, uint32_t sector)
{
...
bool sd_sdio_begin(sd_card_t *sd_card_p)
{
    ...
    if (reply != 0x1AA || status != SDIO_OK)
    {
        ...
        EMSG_PRINTF("%s,%d SDIO not responding to CMD8 SEND_IF_COND, status 0x%x reply 0x%x\n", 
    ...
    sd_card_p->state.card_type = sd_sdio_type(sd_card_p);
    DBG_PRINTF("SDIO card type=%u sectors=%lu OCR=0x%08lx\n",
               sd_card_p->state.card_type,
               (unsigned long)sd_card_p->state.sectors,
               (unsigned long)STATE.ocr);
...
bool sd_sdio_writeSector(sd_card_t *sd_card_p, uint32_t sector, const uint8_t* src)
{
    ...
    uint32_t reply;
    if (/* !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, 16, 512, &reply)) || // SET_BLOCKLEN */
        !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, CMD24_WRITE_BLOCK, sd_sdio_card_address(sd_card_p, sector), &reply)) || // WRITE_BLOCK
    ...
    {
        EMSG_PRINTF("sd_sdio_writeSector(%u) failed: %s (%d)\n", 
...
bool sd_sdio_writeSectors(sd_card_t *sd_card_p, uint32_t sector, const uint8_t *src, size_t n) {
    ...
    if (STATE.ongoing_wr_mlt_blk && sector == STATE.wr_mlt_blk_cnt_sector) {
        ...
        uint32_t reply;
        if (!checkReturnOk(rp2040_sdio_command_R1(sd_card_p, CMD25_WRITE_MULTIPLE_BLOCK, sd_sdio_card_address(sd_card_p, sector), &reply)) ||
    ...
    if (STATE.error != SDIO_OK) {
        EMSG_PRINTF("sd_sdio_writeSectors(,%u,,%u) failed: %s (%d)\n", sector, (unsigned)n, errstr(STATE.error), (int)STATE.error);
...
bool sd_sdio_readSector(sd_card_t *sd_card_p, uint32_t sector, uint8_t* dst)
{
    ...
    if (/* !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, 16, 512, &reply)) || // SET_BLOCKLEN */
        ...
        !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, CMD17_READ_SINGLE_BLOCK, sd_sdio_card_address(sd_card_p, sector), &reply))) // READ_SINGLE_BLOCK
    {
        return false;
    }
    ...
    if (STATE.error != SDIO_OK)
    {
        EMSG_PRINTF("sd_sdio_readSector(,%u,) failed: %s (%d)\n", 
            sector, errstr(STATE.error), (int)STATE.error);
    }
...
bool sd_sdio_readSectors(sd_card_t *sd_card_p, uint32_t sector, uint8_t* dst, size_t n)
{
    ...
    if (/* !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, 16, 512, &reply)) || // SET_BLOCKLEN */
        ...
        !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, CMD18_READ_MULTIPLE_BLOCK, sd_sdio_card_address(sd_card_p, sector), &reply))) // READ_MULTIPLE_BLOCK
    {
        return false;
    }
    ...
    if (STATE.error != SDIO_OK)
    {
        EMSG_PRINTF("sd_sdio_readSectors(%u,...,%u)  failed: %s (%d)\n", 
            sector, (unsigned)n, errstr(STATE.error), (int)STATE.error);
        sd_sdio_stopTransmission(sd_card_p, true);
        return false;
    }
